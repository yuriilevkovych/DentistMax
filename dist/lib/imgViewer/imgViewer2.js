 ...
 */
			self.view = $("<div class='viewport' />").uniqueId().appendTo( self.options.target );
			var $view = $(self.view);
			self.map  = {};
			self.bounds = {};
...
//			cache the image padding information
					...
					if ( 'auto' != self.options.width ) width = self.options.width;
					if ( 'auto' != self.options.height ) height = self.options.height;
/*
...
//			add the leaflet map
					...
					self.map = L.map($view.attr('id'), {crs:L.CRS.Simple,
														...
														inertia: false,
														zoomSnap: 0,
														wheelPxPerZoomLevel: Math.round(48/self.options.zoomStep),
					...
					self.map.on('zoomstart zoom zoomend', function(ev) {
						self.options.onZoom.call(self, ev);
					});
